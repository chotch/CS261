 ...
 * address below:
 *
 * Name: Christien Hotchkiss
 * Email: hotchkch@oregonstate.edu
 */
...
struct student* deep_copy_student(struct student* student) {
	assert(student != NULL);
	int i = 0;
	struct student* deepCopyStudent = malloc(sizeof(struct student*));
	deepCopyStudent->name = malloc(sizeof(char*));	
	while (deepCopyStudent->name[i] != '\0'){
		deepCopyStudent->name[i] = student->name[i];
		i++;
	}
//	(deepCopyStudent->name) = (student->name);
	deepCopyStudent->id = student->id;
	deepCopyStudent->gpa = student->gpa;
...
struct student* create_student_array(int num_students, char** names, int* ids,
	...
	struct student* studentArray = malloc(num_students * sizeof(struct student));
	int i = 0;
	for (i = 0; i<num_students; i++){
		//printf(names[i]);
		init_student(&(studentArray[i]), names[i], ids[i], gpas[i]);	
	}	
...
void print_students(struct student* students, int num_students) {
	...
	assert(students != NULL);
	for (i = 0; i < num_students; i++){
		printf("Name: %s, ID: %d, GPA: %f", students[i].name, students[i].id, students[i].gpa );
		printf("\n");
...
struct student* find_min_gpa(struct student* students, int num_students) {
	assert(students != NULL);
	struct student* lowestGPA;
	int i = 0;
	float lowGPA = 4.0;
	for (i = 0; i< num_students; i++){
		if (students[i].gpa < lowGPA){
			lowestGPA = &students[i];
			lowGPA = students[i].gpa;
		}
	} 
...
void sort_by_gpa(struct student* students, int num_students) {
	...
	for (i = 0; i<num_students-1; i++){
		...
		for (j = 0; j<num_students-i-1; j++){
			if(students[j+1].gpa > students[j].gpa){
				//temp = deep_copy_student(&(students[j]));
				//students[j] = *(deep_copy_student(&(students[j+1])));
				//students[j+1] = *temp;
				*temp = (students[j]);
	...
	/*for (i = 0; i< num_students - 1; i++){
		...
		for (j = i + 1; j<num_students; j++){
			if (students[j].gpa < students[key].gpa)
				key = j;
		}
		*temp = students[j];
		students[j] = students[j+1];
...
/*	for (i = 1; i<num_students; i++){
		...
		while (j>=0 && students[j].gpa > students[key].gpa){
			students[j+1] = students[j];
			j = j - 1;

		}

		students[j+1] = students[key];
	}*/
